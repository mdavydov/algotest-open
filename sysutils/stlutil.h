...
struct range_t
{
    ...
    T m_begin, m_end;
    constexpr range_t(T begin, T end) : m_begin(begin), m_end(end) {}
...
/// Provides python-like enumeration
/// by creating iterable object that wraps another iterable
/// and generates the indices during iteration
/// Usage for iterable <things>:
/// for (auto [i, thing] : enumerate(things)) { ... }
template <typename T,
          typename IndexType,
          typename TIter = decltype(std::begin(std::declval<T>())),
          typename = decltype(std::end(std::declval<T>()))>
constexpr auto enumerate(IndexType start, T&& iterable )
{
    struct iterator
    {
        IndexType i;
