...
namespace cnpy {
    ...
    template<typename T> void npy_save(std::string fname, const T* data, const std::vector<size_t> shape, std::string mode = "w") {
        ...
        if(fp) {
            ...
            if(word_size != sizeof(T)) {
                LOGI_("libnpy error: "<<fname<<" has word size "<<word_size<<" but npy_save appending data sized "<<sizeof(T));
                ASSERT( word_size == sizeof(T) );
            }
            if(true_data_shape.size() != shape.size()) {
                LOGI_("libnpy error: npy_save attempting to append misdimensioned data to "<<fname);
                ASSERT(true_data_shape.size() != shape.size());
            }
            
            for(size_t i = 1; i < shape.size(); i++) {
                if(shape[i] != true_data_shape[i]) {
                    LOGI_("libnpy error: npy_save attempting to append misshaped data to "<<fname);
    ...
    template<typename T> void npz_save(std::string zipname, std::string fname, const T* data, const std::vector<size_t>& shape, std::string mode = "w")
    {
        ...
        uLong crc = crc32(0L, (uint8_t*)&npy_header[0], uInt(npy_header.size()));
